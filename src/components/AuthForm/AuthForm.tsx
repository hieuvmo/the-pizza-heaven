...
export const AuthForm: React.FC<AuthFormProps> = ({ children, imageSide, imageLink }) => {
  return (
    <div className="auth-wrapper">
      ...
      <div className={`auth-container ${imageSide === ImageSide.LEFT ? '' : 'flex-row-reverse'} `}>
        <div className={`auth-img_side ${imageSide === ImageSide.LEFT ? 'auth-img_side-left' : 'auth-img_side-right'}`}>
          <img
            src={imageLink}
            alt=""
            style={{ width: '100%', height: '100%', objectFit: 'contain', padding: '4rem' }}
            loading="eager"
          />
        </div>
